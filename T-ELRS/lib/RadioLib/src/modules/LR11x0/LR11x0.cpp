...
int16_t LR11x0::setRfHfPin(int16_t hf_tx_pin, int16_t hf_rx_pin) {
  if(hf_tx_pin < 0)
  {
    return RADIOLIB_ERR_NONE;
  }
  _hf_tx_pin = hf_tx_pin;
  this->mod->hal->pinMode(_hf_tx_pin, this->mod->hal->GpioModeOutput);
  this->mod->hal->digitalWrite(_hf_tx_pin, this->mod->hal->GpioLevelLow);
  if(hf_rx_pin < 0)
  {
    return RADIOLIB_ERR_NONE;
  }
  _hf_rx_pin = hf_rx_pin;
  this->mod->hal->pinMode(_hf_rx_pin, this->mod->hal->GpioModeOutput);
  this->mod->hal->digitalWrite(_hf_rx_pin, this->mod->hal->GpioLevelLow);
  return 1;
}

int16_t LR11x0::rf_hf_pin_control(OpMode_t mode) {
  if(mode == MODE_END_OF_TABLE) {
  ...
  switch(mode) {
    ...
    case MODE_RX:
      this->mod->hal->digitalWrite(_hf_tx_pin, this->mod->hal->GpioLevelLow);
      this->mod->hal->digitalWrite(_hf_rx_pin, this->mod->hal->GpioLevelHigh);
      break;
    case MODE_TX_HF:
      this->mod->hal->digitalWrite(_hf_tx_pin, this->mod->hal->GpioLevelHigh);
      this->mod->hal->digitalWrite(_hf_rx_pin, this->mod->hal->GpioLevelLow);
      break;
  } 
  int tx_pin = this->mod->hal->digitalRead(_hf_tx_pin);
  int rx_pin = this->mod->hal->digitalRead(_hf_rx_pin);
